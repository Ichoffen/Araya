...
export async function chatProxy(req, res) {
  try {
    ...
    if (!apiKey) { res.status(400).json({ error: 'API key is not set. Open Settings and paste your key.' }); return; }
    ...
    if (!upstream.ok || !upstream.body) { const text = await upstream.text(); res.status(500).send(text); return; }
    ...
    while (true) {
      ...
      for (const block of chunk.split('\n\n')) {
        ...
        try { const obj = JSON.parse(data); const txt = obj.output_text ?? ''; if (txt) { acc += txt; res.write(`data: ${JSON.stringify({ delta: txt })}\n\n`); } }
        catch { acc += data; res.write(`data: ${JSON.stringify({ delta: data })}\n\n`); }
      }
    }
